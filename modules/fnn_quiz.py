...
def test(cfg, model, valid_data_loader):
    ...
    with torch.no_grad():
        preds = []
        for data in tqdm(valid_data_loader, total=len(valid_data_loader) / cfg.batch_size, desc='test'):
            # 1. Forward
            pred = model(data)
...
real_quiz_set['target'] = pd.Series(np.round(torch.argmax(final_day, dim=1)))
