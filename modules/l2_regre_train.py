...
def run(cfg, train_dataset, valid_dataset, fp):
    ...
    # Build model.
    model = L2Regre(train_dataset.__feature_len__())
    # Build optimizer.
...
savedf.to_csv('data/sl_data/l2Regre_train.tsv', sep='\t', index=None) 

lao = np.array([1 / x for x in loss_and_output])
lao = lao / lao.sum()

json.dump(list(lao), open('para/l2Regre_weight.json', 'w'))
json.dump(loss_and_output, open('para/all_l2Regre_log.json', 'w'))
